  ...
  class Assert 
    ...
    class_module.module_eval {
      ...
      const_set_lazy(:AssertionFailedException) { Class.new(RuntimeException) do
        ...
        class_module.module_eval {
          ...
          # @since 3.1
          const_set_lazy(:SerialVersionUID) { 3257852073508024376 }
    ...
    class_module.module_eval {
      ...
      def is_legal(expression, message)
        if (!expression)
          raise IllegalArgumentException.new("assertion failed; " + message) # $NON-NLS-1$
        end
        return expression
      end
      ...
      def is_not_null(object, message)
        if ((object).nil?)
          raise AssertionFailedException.new("null argument;" + message) # $NON-NLS-1$
        end
      end
      ...
      def is_true(expression, message)
        if (!expression)
          raise AssertionFailedException.new("Assertion failed: " + message) # $NON-NLS-1$
        end
        return expression
      end
    }
